{
  Float_t ref[] = {5.62246, 5.58650, 5.60921};
  Float_t eref[] = {0.01143, 0.01070, 0.00601};
  Float_t dc[] = {-0.13022, -0.12958, -0.11066};
  Float_t edc[] = {0.00059, 0.00118, 0.00086};
  ...
  for(int i = 0; i < n; ++i) {
    ly_EJ200PS_norm[i] = ly_EJ200PS[i]/ref[i];
    ely_EJ200PS_norm[i] = sqrt(pow(ely_EJ200PS[i]/ref[i],2)+pow(ly_EJ200PS[i]*eref[i]/pow(ref[i],2),2));
    ly_EJ200PVT_norm[i] = ly_EJ200PVT[i]/ref[i];
    ely_EJ200PVT_norm[i] = sqrt(pow(ely_EJ200PVT[i]/ref[i],2)+pow(ly_EJ200PVT[i]*eref[i]/pow(ref[i],2),2));
    ly_EJ260PS_norm[i] = ly_EJ260PS[i]/ref[i];
    ely_EJ260PS_norm[i] = sqrt(pow(ely_EJ260PS[i]/ref[i],2)+pow(ly_EJ260PS[i]*eref[i]/pow(ref[i],2),2));
    ly_EJ260PVT_norm[i] = ly_EJ260PVT[i]/ref[i];
    ely_EJ260PVT_norm[i] = sqrt(pow(ely_EJ260PVT[i]/ref[i],2)+pow(ly_EJ260PVT[i]*eref[i]/pow(ref[i],2),2));
  }
  ...
  mgr->GetYaxis()->SetTitle("LY/LY(ref)");
