...
void dcost_vs_ddot_with2020and2021(TString tagTime="") {
  set_root_style();
  Bool_t useGray = true;
  ...
  incMeas.push_back("EJ200PS-1X1P Sept. 2020");
  incMeas.push_back("EJ200PS-1X1P Dec. 2020");
  incMeas.push_back("EJ200PS May 2021");
  incMeas.push_back("EJ200PS Sept. 2021");
  //incMeas.push_back("EJ200PS-1X2P");
  //incMeas.push_back("EJ200PS-1X2P Sept. 2020");
  //incMeas.push_back("EJ200PS-1X2P Dec. 2020");
  //incMeas.push_back("EJ200PS-2X1P");
  //incMeas.push_back("EJ200PS-2X1P Sept. 2020");
  //incMeas.push_back("EJ200PS-2X1P Dec. 2020");
  ...
  measurements.push_back(
    {
      ...
      {7,7,7,1.32,7,1.26,4.2,7,7,1.26,1.71,6,7},
  ...
  measurements.push_back(
    {
      ...
      {46,46,46,46,46,46,46,46,46,46}, // krad/hr
      {4.92632,5.94148,6.65586,5.89128,7.37121,7.46682,7.65451,9.53392,9.17561,10.51769}, // Mrad
      {46*0.05,46*0.05,46*0.05,46*0.05,46*0.05,46*0.05,46*0.05,46*0.05,46*0.05,46*0.05},
      ...
      4,22,1.4,"EJ260PVT-1X1P May 2021",1,NULL
    }
  );
  ...
  measurements.push_back(
    {
      ...
      {"A10"},
      {7.0}, // Mrad
      4,20,1.4,"EJ200PVT-1X1P May 2021",1,NULL
    }
  );
  ...
  measurements.push_back(
    {
      ...
      {"L11R"},
      {7.0}, // Mrad
      4,20,1.4,"EJ200PS May 2021",1,NULL
    }
  ...
  measurements.push_back(
    {
      ...
      {"L7R","L8R"},
      {6.9,6.9}, // Mrad
      2,20,1.4,"EJ200PS Sept. 2021",1,NULL
    }
  );
  ...
  for (vector<measurement>::iterator i = measurements.begin(); i!=measurements.end();++i) {
    ...
    if (useGray) {
      for(int i = 0; i<n; ++i) {
        ...
        y_temp[i]*=0.01*1000; // Mrad to kGy
        ex_temp[i]*=0.01*1000; // krad/hr to Gy/hr
        ey_temp[i]*=0.01*1000; // Mrad to kGy
      }
    }
    ...
    for (vector<TString>::iterator it = incMeas.begin(); it!=incMeas.end();++it){
      if ((*it).CompareTo(i->name) == 0) {
        ...
        for (unsigned int j = 0; j < n; j++) {
          if (abs(i->dose[j]-7) > 0.1) {
            float xtxt = useGray ? i->x[j]*=0.01*1000 : i->x[j];
            float ytxt = useGray ? i->y[j]*=0.01*1000 : i->y[j];
            char desc[20];
            sprintf(desc, "%s %.1f kGy", i->irrFacility[j].Data(), i->dose[j]*0.01*1000);
...
*/
  // Prepare the legend
  TLegend *legendA = new TLegend(0.45,0.16,0.75,0.28);
  ...
  canv->SaveAs("paperPlots/ver14/detailed/"+filename+".pdf");
}
