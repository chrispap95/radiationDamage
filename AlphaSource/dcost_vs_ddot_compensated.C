...
void dcost_vs_ddot_compensated(TString tagTime="") {
    set_root_style();
    Bool_t useGray = true;
    ...
    incMeas.push_back("EJ260PVT-2X1P Sept. 2020");
    incMeas.push_back("EJ260PVT-2X1P Dec. 2020");
    ...
    measurements.push_back(
        {
            ...
            {"N4","N5","N8","N9","N15","N16","T1_2","T1_3","T1_5","T1_N1","AO-1_1","AO-1_2","AO-1_3"},
...
////////////////////////////////////////////////////////////////////////////////
    ...
    for (vector<measurement>::iterator i = measurements.begin(); i!=measurements.end();++i) {
        ...
        // Convert to 7 Mrad dose equivalent
        const float slope = 1.214;
        const float sigma_slope = 0.245;
        const float equivalent_dose = useGray ? 0.01*1000*1000*7 : 7;
        const float x_lim = useGray ? 0.01*1000*1 : 1;
        for(int i = 0; i<n; ++i) {
          float dose_diff=(equivalent_dose-dose_temp[i]);
          if (dose_diff>0.1 && x_temp[i]>x_lim){
            y_temp[i]+=slope*dose_diff;
            ey_temp[i]=sqrt(pow(ey_temp[i],2)+pow(dose_diff*sigma_slope,2)+pow(slope,2)*(pow(equivalent_dose*0.012,2)+pow(dose_temp[i]*0.012,2)));
          }
        }
    ...
    canv->SaveAs("paperPlots/ver13/"+filename+"_comp.pdf");
}
